(() => {
  ...
  const meanings={
    ...
    satisfaction:['顺序尺度（Ordinal）','满意度 4 通常比 2 高，但 4 与 2 的差距不一定等同于 3 与 1；数字常是在编码顺序。'],
  ...
  const scaleInfo=[
    ...
    ['顺序尺度（Ordinal）','除了相同 / 不同，还可以比较先后、高低或等级；但相邻等级之间的距离不一定相等。','<strong>满意度：</strong>5 &gt; 3 ✓<br>但 5 − 3 与 3 − 1 是否等距？未必'],
  ...
  const impactText={summary:'名义尺度（Nominal）常看频数和众数；顺序尺度（Ordinal）可以讨论中位数和顺序；区间尺度和比率尺度才适合更广泛的算术统计。',encoding:'把类别写成 1/2/3 只是编码。若模型把这些数当连续数值，就可能误以为存在顺序和等距关系。',distance:'“两个城市差 2”没有意义；但“两个身高差 10 cm”有意义。距离函数必须尊重属性类型。',model:'不同模型对类别、有序和连续属性的处理方式不同。输入模型之前，必须先理解属性语义。'};
