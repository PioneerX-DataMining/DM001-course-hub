(() => {
  ...
  const meanings={
    id:['Nominal-like identifier','学号虽然由数字组成，但主要作用是标识对象。2026002 − 2026001 = 1，并没有“差 1 个单位”的分析意义。'],
    satisfaction:['Ordinal','满意度 4 通常比 2 高，但 4 与 2 的差距不一定等同于 3 与 1；数字常是在编码顺序。'],
    temp:['Interval','25°C 比 15°C 高 10°C，差值有意义；但 20°C 不能说是 10°C 的“两倍热”。'],
    height:['Ratio','175 cm 的差值和比例都具有清晰含义；100 cm 可以说是 50 cm 的两倍。']
  };
  ...
  const scaleInfo=[
    ['Nominal · 名义尺度','只允许判断“相同 / 不同”。类别没有天然顺序。','<strong>血型：</strong>A ≠ B ✓<br>但 A &gt; B ✗'],
    ['Ordinal · 有序尺度','除了相同 / 不同，还可以比较先后、高低或等级；但相邻等级之间的距离不一定相等。','<strong>满意度：</strong>5 &gt; 3 ✓<br>但 5 − 3 与 3 − 1 是否等距？未必'],
    ['Interval · 区间尺度','顺序与差值都有意义，但零点不是绝对“没有”，因此比值通常没有意义。','<strong>摄氏温度：</strong>20°C − 10°C = 10°C ✓<br>20°C = 2 × 10°C ✗'],
    ['Ratio · 比率尺度','除了前面的操作，还具有有意义的绝对零点，因此比例也可以解释。','<strong>身高：</strong>100 cm − 50 cm = 50 cm ✓<br>100 cm = 2 × 50 cm ✓']
  ];
  ...
  const impactText={summary:'Nominal 常看频数和众数；Ordinal 可以讨论中位数和顺序；真正的数值尺度才适合更广泛的算术统计。',encoding:'把类别写成 1/2/3 只是编码。若模型把这些数当连续数值，就可能误以为存在顺序和等距关系。',distance:'“两个城市差 2”没有意义；但“两个身高差 10 cm”有意义。距离函数必须尊重变量类型。',model:'不同模型对类别、有序和连续变量的处理方式不同。输入模型之前，必须先理解变量语义。'};
